...
st.set_page_config(
    page_title="Tự chọn _ So sánh độ sai lệch của 2 màu trong cùng hình ảnh", layout="wide", page_icon="😽" )
...
st.markdown("<h1 style='text-align: center;'>Độ sai lệch và sự phối trộn màu sắc</h1>", unsafe_allow_html=True)
...
if conversion == "Độ sai lệch của 2 màu ở 2 hình ảnh khác nhau":
    ...
    uploaded_file1 = st.file_uploader("Tải ảnh đầu tiên", type=["png", "jpg", "jpeg", "tif"])
...
if conversion == "Phối trộn màu":
    ...
    st.title("Ứng dụng phối trộn màu (RGB)")
    ...
    st.markdown("---")
    st.subheader("Kết quả phối trộn màu")
    st.markdown(f"- **RGB:** `{mixed_rgb}`")
    st.markdown(f"- **HEX:** `{mixed_hex}`")
    st.markdown(f"- **CMYK:** `{mixed_cmyk}`")

    # Màu kết quả hiển thị RGB
